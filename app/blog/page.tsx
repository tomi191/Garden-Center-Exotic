...
export default async function BlogPage() {
  ...
  return (
    <>
      <BreadcrumbJsonLd items={[{ name: "Блог", href: "/blog" }]} />
      {/* Hero Section */}
        ...
        <Container>
          ...
          <div className="mt-12 text-center">
            <p className="text-[var(--color-gray-600)]">
              {posts.length} статии налични. Редовно публикуваме ново съдържание!
            </p>
          </div>
        </Container>
      </Section>
                ...
                <Card key={index} className="bg-white text-center cursor-pointer hover:shadow-xl transition-all group">
                  <CardContent className="py-8">
                    ...
                    <p className="text-sm text-[var(--color-gray-600)] mt-2">
                      {posts.filter((p) => p.category === category).length} статии
                    </p>
                  </CardContent>
                </Card>
              ))}
