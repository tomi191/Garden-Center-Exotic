                ...
                <>
                  ...
                  <div className="absolute right-0 mt-1 w-48 bg-[#2a2a2a] rounded-lg shadow-xl border border-white/10 py-1 z-50">
                    <div className="px-3 py-2 border-b border-white/10">
                      <p className="text-[11px] font-medium text-white/90 truncate">{user.name || "Admin"}</p>
                      <p className="text-[10px] text-white/40 truncate">{user.email}</p>
                    </div>
                    <Link
                      href="/admin/profile"
                      onClick={() => setProfileOpen(false)}
                      className="flex items-center gap-2 px-3 py-2 text-[11px] text-white/70 hover:bg-white/5"
                    >
                      <User className="w-3 h-3" /> Профил
                    </Link>
                    <button
                      onClick={() => signOut({ callbackUrl: "/admin/login" })}
                      className="w-full flex items-center gap-2 px-3 py-2 text-[11px] text-red-400 hover:bg-white/5"
                    >
